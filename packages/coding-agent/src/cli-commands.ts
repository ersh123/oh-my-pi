...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	if (invalidNikoflowDepth) {
		return { error: `Invalid Nikoflow depth: ${invalidNikoflowDepth}. Use ${NIKOFLOW_DEPTHS.join(", ")}.` };
	}
	...
	if (invalidLeadingNikoflowDepth) return { error: `Invalid Nikoflow depth: ${invalidLeadingNikoflowDepth}. Use ${NIKOFLOW_DEPTHS.join(", ")}.` };
