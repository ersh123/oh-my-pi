...
/**
 * Adapts the already-running interactive TUI's selector to Nikoflow's model-role
 * picker. Unlike `selectNikoflowModelRole`, it never creates a competing
 * ProcessTerminal/TUI over the same stdin.
 */
export function createInteractiveNikoflowRolePicker(
	select: (
		title: string,
	...
	return async request => {
		...
		return request.options.find(option => option.label === selected)?.selector ?? null;
	};
}
