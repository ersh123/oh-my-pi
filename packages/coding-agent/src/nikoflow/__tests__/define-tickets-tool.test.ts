...
function ticketizationState() {
	return advancePhase(advancePhase(advancePhase(createState("max"))));
}
...
describe("nikoflow_define_tickets tool", () => {
	...
	test("asks the TUI to activate Nikoflow instead of prescribing a shell command", async () => {
		let activations = 0;
		const session = {
			requestNikoflowActivation: async () => {
				activations += 1;
				return true;
			},
			defineNikoflowTickets(_tickets: readonly NikoflowTicketInput[]): NikoflowTicketDefinitionResult {
				return { tickets: [], errors: ["Nikoflow mode is not active."] };
			},
		} satisfies Partial<ToolSession>;
		...
		expect(text).toContain("Nikoflow was enabled in the current interactive session");
		expect(text).not.toContain("nikoflow:max");
	});
});
